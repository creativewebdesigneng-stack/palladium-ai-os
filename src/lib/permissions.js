import { isDevAdminEmail, getStoredPlan } from '@/lib/access';
...
const FREE_FEATURES = {
  marketplace: true, browseAgents: true, viewAgents: true, viewUpcomingAgents: true,
  viewProfiles: true, viewWorkflows: true, viewCommunityAgents: true, viewAnnouncements: true,
  personalDashboard: true,
  createAgents: false, deployAgents: false, runTasks: false, createWorkflows: false,
  advancedWorkflows: false, advancedTools: false, privateMarketplace: false,
  memory: false, advancedMemory: false, integrations: false, apiAccess: false,
  agentBuilder: false, templates: false, analytics: false, marketplaceAccess: false,
  teamWorkspace: false, advancedSecurity: false, customIntegrations: false,
  unlimitedTeams: false, priorityInfrastructure: false,
};

// Pro "Builder" — create/deploy agents, basic workflows + memory, analytics.
const PRO_FEATURES = {
  ...FREE_FEATURES,
  createAgents: true, deployAgents: true, runTasks: true, createWorkflows: true,
  memory: true, agentBuilder: true, templates: true, analytics: true,
  marketplaceAccess: true, integrations: true,
};
...
const BUSINESS_FEATURES = {
  ...PRO_FEATURES,
  advancedWorkflows: true, advancedMemory: true, advancedTools: true,
  privateMarketplace: true, apiAccess: true, teamWorkspace: true,
  // Enterprise controls unlocked at £1,500/mo
  auditLogs: true, loginTracking: true, permissionManagement: true,
  departments: true, roleManagement: true, multipleTeams: true,
};
...
const ENTERPRISE_FEATURES = {
  ...BUSINESS_FEATURES,
  advancedSecurity: true, customIntegrations: true, unlimitedTeams: true,
  priorityInfrastructure: true, priorityFeatures: true, customRoles: true,
  sso: true, advancedAuditLogs: true, dataResidency: true,
};
...
export const PLAN_LIMITS = {
  free:       { agents: 0,   users: 1,   monthlyRuns: 0,      storageGB: 1,   teams: 0, departments: 0,  apiKeys: 0,   retentionDays: 0 },
  pro:        { agents: 5,   users: 1,   monthlyRuns: 500,    storageGB: 5,   teams: 1, departments: 0,  apiKeys: 2,   retentionDays: 30 },
  business:   { agents: 50,  users: 25,  monthlyRuns: 50000,  storageGB: 100, teams: 10, departments: 10, apiKeys: 25,  retentionDays: 180 },
  enterprise: { agents: null, users: null, monthlyRuns: null, storageGB: 500, teams: null, departments: null, apiKeys: null, retentionDays: 365 },
};
...
export function getPlanKey(user) {
  if (user && isDevAdminEmail(user.email)) return 'enterprise';
  // The SDK nests custom User fields under `data` — read both shapes so the
  // backend-stored subscription_plan is the source of truth (falls back to the
  // dev-only localStorage plan for the mock builder session).
  const p = user?.data?.subscription_plan || user?.subscription_plan || getStoredPlan();
  return PLAN_TIER_ORDER.includes(p) ? p : 'free';
}

export function isFree(user) { return getPlanKey(user) === 'free'; }
...
export function getUsageLimits(user) {
  const key = getPlanKey(user);
  const map = {
    free: { activeAgents: { used: 0, max: 0 }, monthlyRuns: { used: 0, max: 0 }, storage: { used: 0.2, max: 1, unit: 'GB' } },
    pro: { activeAgents: { used: 3, max: 5 }, monthlyRuns: { used: 210, max: 500 }, storage: { used: 1.8, max: 5, unit: 'GB' } },
    business: { activeAgents: { used: 38, max: 50 }, monthlyRuns: { used: 41000, max: 50000 }, storage: { used: 78, max: 100, unit: 'GB' } },
    enterprise: { activeAgents: { used: 86, max: 200 }, monthlyRuns: { used: 45000, max: 250000 }, storage: { used: 120, max: 500, unit: 'GB' } },
  };
