...
export const retailReceptionistInquirySchema = z.object({
  workspace_id: uuid,
...
export type RetailReceptionistInquiry = z.infer<typeof retailReceptionistInquirySchema>;
...
const SYSTEM_PROMPT = [
  ...
  'Respect the receptionist profile capability flags. If a topic is disabled, say staff need to handle that topic instead of revealing omitted data.',
  'Stock facts are intentionally reduced to customer-safe availability states. Never infer or reveal internal counts.',
  'Never reveal another customer’s data. Order or appointment details are supplied only when the server has verified supplied customer contact against that record.',
...
function selectRelevantCatalog(question: string, catalog: Row[], inventory: Row[]) {
  ...
  for (const level of inventory) {
    stock.set(String(level.item_id), (stock.get(String(level.item_id)) ?? 0) + Number(level.on_hand ?? 0) - Number(level.reserved ?? 0));
  }
  const scored = catalog.map((item) => {
    const haystack = `${item.name ?? ''} ${item.sku ?? ''} ${item.barcode ?? ''} ${item.category ?? ''} ${item.description ?? ''}`.toLowerCase();
    const score = terms.reduce((sum, term) => sum + (haystack.includes(term) ? 1 : 0), 0);
    const available = item.track_inventory ? Math.max(0, stock.get(String(item.id)) ?? 0) : null;
    const reorderPoint = Math.max(0, Number(item.reorder_point ?? 0));
    const availability = !item.track_inventory
      ? 'not_tracked'
      : available === 0
        ? 'out_of_stock'
        : available !== null && available <= Math.max(1, reorderPoint)
          ? 'low_stock'
          : 'in_stock';
    return {
      id: item.id,
...
function sanitizePayload(proposal: z.infer<typeof proposalSchema>, data: RetailReceptionistInquiry) {
...
export async function runRetailReceptionistCore(input: {
  sb: Sb;
  userId: string;
  data: RetailReceptionistInquiry;
  surface?: string;
}) {
  const { sb, userId } = input;
  const data = retailReceptionistInquirySchema.parse(input.data);
  const workspaceId = data.workspace_id;
  const surface = input.surface ?? 'retail_receptionist';

  try {
    const entitlements = await getEntitlements(sb, userId);
  ...
  const workspaceResult = await sb.from('retail_workspaces')
    .select('id,business_name,business_type,currency,timezone,phone,email,address')
  ...
  const profile = await resolveProfile(sb, userId, workspaceId, nullify(data.profile_id), nullify(data.location_id));
  const now = new Date();

  const [preferenceResult, locationsResult, catalogResult, inventoryResult, callResult] = await Promise.all([
    sb.from('user_ai_preferences').select('default_provider,default_model').eq('user_id', userId).maybeSingle(),
    sb.from('retail_locations').select('id,name,kind,phone,address,active').eq('workspace_id', workspaceId).eq('user_id', userId).eq('active', true).order('name'),
    sb.from('retail_catalog_items').select('id,name,item_type,sku,barcode,category,description,sale_price,currency,service_duration_minutes,track_inventory,reorder_point,active').eq('workspace_id', workspaceId).eq('user_id', userId).eq('active', true).limit(500),
    sb.from('retail_inventory_levels').select('item_id,location_id,on_hand,reserved').eq('workspace_id', workspaceId).eq('user_id', userId).limit(2000),
    data.call_id
      ? sb.from('retail_call_inbox').select('id,reason,status,needs_follow_up,appointment_id,source,received_at').eq('id', data.call_id).eq('workspace_id', workspaceId).eq('user_id', userId).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ]);
  const loadError = [locationsResult, catalogResult, inventoryResult, callResult].find((result: any) => result.error)?.error;
  ...
  try {
    ...
    for (const toolCall of result.toolCalls.filter((tool) => tool.name === proposalTool.name).slice(0, 3)) {
      ...
      const { data: queued, error } = await sb.from('retail_reception_actions').insert({
        ...
        source: surface === 'retail_phone_receptionist' ? 'voice' : 'ai',
    ...
    const evidence = [
      ...
      ...(capabilities.answer_stock ? ['inventory_state'] : []),
    ...
    await recordUsage({
      ...
      quantity: 1,
      metadata: {
        surface, provider: result.provider, model: result.model, preference_source: preferenceSource,
    ...
    await writeAudit({
      userId,
      action: surface === 'retail_phone_receptionist' ? 'retail.receptionist.phone_turn' : 'retail.receptionist.inquiry',
    ...
    return {
      ...
      queuedActions,
      blockedProposals,
      profile: profile ? { id: String(profile.id), name: String(profile.name) } : null,
      verification: {
