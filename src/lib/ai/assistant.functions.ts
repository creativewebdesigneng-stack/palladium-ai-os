...
const SYSTEM_PROMPT = [
  ...
  "For questions that require current or private workspace data that has not been provided to you, clearly say what you do not know rather than inventing facts.",
...
async function runAssistantWithFallback(args: {
  ...
  try {
    const primary = await runChat({ provider: args.provider, model: args.model, messages: args.messages, maxTokens: 900 });
    ...
    const fallback = await runChat({ provider: "groq", model: fallbackModel, messages: args.messages, maxTokens: 900 });
...
export const assistantChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      const preferenceResult = await sb.from("user_ai_preferences").select("default_provider,default_model").eq("user_id", context.userId).maybeSingle();
    ...
    try {
      ...
      await recordUsage({ userId: context.userId, metric: "assistant_message", quantity: 1, metadata: { provider: result.provider, model: result.model, preference_source: preferenceSource, fallback_from: result.fallbackFrom ?? null, input_tokens: result.usage.input, output_tokens: result.usage.output } });
      await writeAudit({ userId: context.userId, action: "assistant.message", targetType: "assistant", status: "success", metadata: { provider: result.provider, model: result.model, preferenceSource, fallbackFrom: result.fallbackFrom ?? null } });
      ...
      await writeAudit({ userId: context.userId, action: "assistant.message", targetType: "assistant", status: "failed", metadata: { provider, model, preferenceSource, status, error: error instanceof Error ? error.message : String(error) } });
