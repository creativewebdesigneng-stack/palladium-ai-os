...
async function resolvePlan(admin: any, userId: string, orgId: string | null): Promise<string> {
  const query = admin
    ...
    .limit(1);
  const { data } = orgId ? await query.eq('org_id', orgId) : await query.is('org_id', null).eq('user_id', userId);
...
export async function authenticateApiRequest(
  ...
  if (!presented) {
    throw new ApiError(401, 'missing_api_key', 'Provide your API key as `Authorization: Bearer pk_live_...`.');
  }
  ...
  if (scopes.length && !scopes.includes(options.scope)) {
    throw new ApiError(403, 'insufficient_scope', `This key is missing the \`${options.scope}\` scope.`, {
  ...
  if (options.execution && !limits.execution) {
    throw new ApiError(403, 'plan_execution_disabled', `The ${limits.label} plan does not include the execution API. Upgrade to Builder or above.`, {
      plan: planCode,
    });
  }
  ...
  const [{ count: minuteCount }, { count: dayCount }] = await Promise.all([
    admin.from('api_request_logs').select('id', { count: 'exact', head: true }).eq('user_id', key.user_id).gte('created_at', minuteAgo),
    admin.from('api_request_logs').select('id', { count: 'exact', head: true }).eq('user_id', key.user_id).gte('created_at', dayAgo),
  ]);

  if ((minuteCount ?? 0) >= limits.requestsPerMinute) {
    throw new ApiError(429, 'rate_limit_exceeded', `Rate limit of ${limits.requestsPerMinute} requests/minute for the ${limits.label} plan was exceeded.`, {
  ...
  if ((dayCount ?? 0) >= limits.requestsPerDay) {
    throw new ApiError(429, 'daily_quota_exceeded', `Daily quota of ${limits.requestsPerDay} requests for the ${limits.label} plan was exceeded.`, {
  ...
  if (options.execution) {
    ...
    if ((execCount ?? 0) >= limits.executionsPerDay) {
      throw new ApiError(429, 'execution_quota_exceeded', `Daily execution quota of ${limits.executionsPerDay} runs for the ${limits.label} plan was exceeded.`, {
...
export function withApiAuth(
  ...
  return async ({ request, params }: { request: Request; params?: any }) => {
    ...
    try {
      ...
      if (ctx) {
        await logRequest({ admin: ctx.admin, request, path: options.path, status, startedAt, ctx, error: message });
      }
...
export function requireString(body: Record<string, any>, field: string, max = 4000): string {
  ...
  if (value.length > max) throw new ApiError(400, 'invalid_request', `\`${field}\` must be ${max} characters or fewer.`);
  return value.trim();
}
