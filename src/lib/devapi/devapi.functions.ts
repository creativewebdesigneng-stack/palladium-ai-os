...
export const listApiKeysFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await sb
      .from('api_keys')
      .select('id,name,environment,key_prefix,last_four,scopes,request_count,created_at,last_used_at,revoked_at')
...
export const createApiKeyFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { count } = await sb.from('api_keys').select('id', { count: 'exact', head: true }).is('revoked_at', null);
    if ((count ?? 0) >= limits.maxKeys) {
      throw new Error(`The ${limits.label} plan allows ${limits.maxKeys} active API key(s). Revoke one or upgrade.`);
    }
...
export const rotateApiKeyFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const generated = await generateApiKey((existing.environment === 'test' ? 'test' : 'live') as 'live' | 'test');
    const { data: row, error } = await sb
      .from('api_keys')
      .update({
        key_hash: generated.hash,
...
export const listWebhooksFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: hooks }, { data: deliveries }] = await Promise.all([
      sb
        .from('webhooks')
        .select('id,url,events,is_active,name,secret_prefix,delivery_count,failure_count,last_delivery_at,created_at')
        .order('created_at', { ascending: false }),
      sb
    ...
    for (const d of deliveries ?? []) if (!lastStatus.has(d.webhook_id)) lastStatus.set(d.webhook_id, d.response_status);

    return {
      webhooks: (hooks ?? []).map((h: any) => ({ ...shapeHook(h), last_response_status: lastStatus.get(h.id) ?? null })),
...
export const createWebhookFn = createServerFn({ method: 'POST' })
  ...
  .inputValidator((input: { url: string; events?: string[]; description?: string }) => {
    ...
    return { url: url.slice(0, 500), events, description: String(input?.description ?? '').slice(0, 200) };
  })
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    const planCode = await resolvePlan(sb, context.userId);
    ...
    const { data: row, error } = await sb
      ...
      .select('id,url,events,is_active,name,secret_prefix,delivery_count,failure_count,last_delivery_at')
      .single();
    if (error) throw new Error(error.message);
...
export const updateWebhookFn = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { webhook_id: string; status?: string; events?: string[]; url?: string }) => ({
...
export const deleteWebhookFn = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { webhook_id: string }) => ({ webhook_id: String(input?.webhook_id ?? '') }))
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
