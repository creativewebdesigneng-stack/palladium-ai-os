...
export async function sendTestDelivery(webhookId: string, userId: string) {
  ...
  const signature = hook.signing_secret ? await hmacSha256Hex(hook.signing_secret, `${timestamp}.${body}`) : null;
  ...
  await admin
    .from('webhooks')
    .update({ last_delivery_at: new Date().toISOString(), delivery_count: (Number(hook.delivery_count) || 0) + 1 })
    .eq('id', hook.id);
