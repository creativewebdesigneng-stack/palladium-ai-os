...
export const listTradingWorkspace = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const [watchlistsResult, itemsResult, journalResult, simulationsResult] = await Promise.all([
      ...
      sb.from('trading_watchlist_items').select('id,watchlist_id,symbol,name,market,asset_type,notes,created_at,updated_at').order('updated_at', { ascending: false }).limit(500),
      sb.from('trading_journal_entries').select('id,symbol,side,status,thesis,plan,outcome,lessons,tags,created_at,updated_at').order('updated_at', { ascending: false }).limit(200),
      sb.from('trading_simulations').select('id,symbol,side,quantity,entry_price,exit_price,currency,status,thesis,notes,opened_at,closed_at,created_at,updated_at').order('updated_at', { ascending: false }).limit(200),
    ]);
