  ...
  .handler(async ({ context }) => {
    ...
    return {
      ...
      tools: (catalogue.data ?? []).map((t: any) => ({
        ...
        permission: (permissions.data ?? []).find((p: any) => p.tool === t.slug && !p.agent_id) ?? null,
      })),
...
export const saveToolPermission = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: tool } = await sb.from('tools').select('slug,is_active,min_plan').eq('slug', data.tool).maybeSingle();
    if (!tool) throw new Error('Unknown tool.');

    if (data.agentId) {
      const { data: agent } = await sb.from('personal_agents').select('id').eq('id', data.agentId).maybeSingle();
      if (!agent) throw new Error('That agent is not available to you.');
    }

    const domains = (data.allowedDomains ?? [])
      .map((d) => d.trim().replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/.*$/, '').toLowerCase())
      .filter(Boolean)
      .slice(0, 50);
    ...
    const match = data.agentId
      ? (await sb.from('tool_permissions').select('id').eq('tool', data.tool).eq('agent_id', data.agentId).maybeSingle()).data
      : existing?.[0];
...
export const runToolManually = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { agentId: string; tool: string; input?: Record<string, unknown> }) => input)
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    const userId = context.userId;
    ...
    if (!grant) throw new Error(`"${data.tool}" is not enabled for ${agent.name} on your current plan.`);
    ...
    const result = await executeTool(data.tool, data.input ?? {}, {
      userId,
      orgId,
      agentId: agent.id,
      taskId: null,
