...
export const getFinanceEconomicData=createServerFn({method:"POST"}).handler(async()=>{
 ...
 for(const row of rows){const nums=row.map(x=>Number(x)).filter(Number.isFinite);const value=nums.at(-1);if(value!==undefined&&value>=0&&value<100){latest={date:row[0],value};}}
