...
export default defineTool({
  ...
  description: "Create a new Mission Control task (mission) for the signed-in user, optionally assigned to an agent.",
  inputSchema: {
    ...
    category: z.string().trim().default("custom").describe("Mission category, e.g. shopping, finance, health."),
    agent_id: z.string().uuid().optional().describe("Agent to assign the mission to."),
    priority: z.enum(["low", "normal", "high", "urgent"]).default("normal").describe("Mission priority."),
  ...
  handler: async ({ request, title, category, agent_id, priority, involves_money }, ctx) => {
    ...
    const { data, error } = await supabase
      ...
      .select("id,title,request,category,status,priority,involves_money,requires_approval,created_at")
      .single();
