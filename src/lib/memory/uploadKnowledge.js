 ...
 */
export async function uploadKnowledgeDocument({ file, title, summary, agentId, orgId, scope, category }) {
  ...
  try {
    ...
    if (storagePath) await supabase.storage.from('knowledge').remove([storagePath]).catch(() => {});
    throw error;
  }
}
