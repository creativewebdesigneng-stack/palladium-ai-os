...
export async function embedTexts(
  ...
  const input = texts.map((t) => (t ?? '').replace(/\s+/g, ' ').trim().slice(0, 8000)).filter(Boolean);
    ...
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        lastError = new EmbeddingError(
          res.status === 429 ? 'Embedding rate limit reached — please retry shortly.' : 'The embedding provider is unavailable.',
          true,
        );
        await new Promise((r) => setTimeout(r, 400 * 2 ** attempt));
        continue;
      }
      if (res.status === 402) throw new EmbeddingError('AI credits are exhausted. Top up to keep indexing memory.');
      if (!res.ok) {
