...
function normaliseKpis(value: unknown): AgentKpi[] {
  if (!Array.isArray(value)) return [];
  return value.slice(0, 20).flatMap((item): AgentKpi[] => {
    ...
    const rawDirection = String(row["direction"] ?? "");
    const direction: NonNullable<AgentKpi["direction"]> =
      rawDirection === "higher" || rawDirection === "lower" || rawDirection === "target"
        ? rawDirection
        : "target";
...
export function normaliseOperatingProfile(value: unknown): AgentOperatingProfile {
  ...
  const delegationInput =
    delegationRaw && typeof delegationRaw === "object" && !Array.isArray(delegationRaw)
      ? (delegationRaw as Record<string, unknown>)
      : {};
