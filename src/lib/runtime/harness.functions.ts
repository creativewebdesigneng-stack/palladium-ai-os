...
export const getHarnessOverview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [agents, tasks, approvals, tools, permissions, executions, identities, passports, delegations, a2a] = await Promise.all([
      ...
      sb.from("agent_passports").select("id,identity_id,autonomy_tier,risk_tier,capabilities").order("created_at", { ascending: false }).limit(100),
