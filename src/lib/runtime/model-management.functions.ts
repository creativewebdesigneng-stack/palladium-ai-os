...
function configured(provider: (typeof PROVIDERS)[number]["id"]): boolean {
...
export const getModelRuntimeOverview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const assignments = (agents ?? []).map((agent: any) => ({
      id: String(agent.id),
    ...
    return {
      providers: PROVIDERS.map((provider) => ({ ...provider, configured: configured(provider.id) })),
      assignments,
      usage: [...usage.values()].sort((a, b) => b.runs - a.runs),
      totals: {
        agents: assignments.length,
        activeAgents: assignments.filter((agent) => agent.status === "active").length,
        recentRuns: (tasks ?? []).length,
        recentCostPence: [...usage.values()].reduce((sum, row) => sum + row.costPence, 0),
        configuredProviders: PROVIDERS.filter((provider) => configured(provider.id)).length,
      },
    };
  });
