...
import { createBrowserTool, isDomainAllowed, resolveBrowserProvider } from '@/lib/mission/browser-agent';
...
const REGISTRY: Record<string, ToolImpl> = {
  ...
  http_request: {
    ...
    run: async (input, ctx) => {
      ...
      const res = await fetch(url, {
        method,
        headers: { 'User-Agent': 'PalladiumAI-Agent/1.0', ...(body ? { 'content-type': 'application/json' } : {}) },
  ...
  file_analysis: {
    def: {
      name: 'file_analysis',
      description: "Read a document from the operator's memory vault and return its text for analysis.",
      parameters: {
        type: 'object',
    ...
    run: async (input, ctx) => {
      ...
      let docQuery = ctx.sb.from('memory_documents').select('id,title,mime_type,size_bytes,chunk_count').limit(1);
      docQuery = documentId ? docQuery.eq('id', documentId) : title ? docQuery.ilike('title', `%${title}%`) : docQuery;
      ...
      const { data: chunks } = await ctx.sb
        ...
        .limit(12);
      return {
        document: { id: doc.id, title: doc.title, mime_type: doc.mime_type, chunks: doc.chunk_count },
  ...
  data_analysis: {
    ...
    run: async (input) => {
      ...
      return {
        ...
        median: sorted.length % 2 ? sorted[mid] : round(((sorted[mid - 1] ?? 0) + (sorted[mid] ?? 0)) / 2),
      };
    },
  },
  ...
  email_send: {
    ...
    run: async (input, ctx) => {
      ...
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(to)) return { error: 'A valid recipient address is required.' };
      const { data, error } = await ctx.sb
        .from('approval_requests')
        .insert({
          user_id: ctx.userId,
          ...
          details: { to, subject: str(input['subject']), body: str(input['body']), task_id: ctx.taskId },
          risk_level: 'high',
          status: 'pending',
        })
        .select('id')
  ...
  shopping_search: {
    ...
    run: async (input, ctx) => {
      ...
      try {
        const offers = await tool.search(requirement, { budget, currency: str(input['currency'], 'GBP') });
        const ranked = await tool.compare(budget ? offers.filter((o) => o.price <= budget) : offers);
        return {
          requirement,
          budget,
  ...
  prepare_purchase: {
    ...
    run: async (input, ctx) => {
      ...
      if (!isDomainAllowed(url, allowed)) return { error: 'That seller domain is not on this agent’s allow-list.' };
      ...
      const { data: approval } = await ctx.sb
        ...
        .insert({
          ...
          details: { purchase_request_id: pr?.id, product, seller, itemPrice, deliveryCost, total, currency, url },
      ...
      if (pr?.id && approval?.id) {
        await ctx.sb.from('purchase_requests').update({ approval_request_id: approval.id }).eq('id', pr.id);
      }
      return {
        prepared: true,
  ...
  database_query: {
    def: {
      ...
      parameters: {
        ...
        properties: {
          table: {
            type: 'string',
            enum: ['personal_tasks', 'personal_agents', 'agent_memories', 'purchase_requests', 'agent_tasks'],
          },
          limit: { type: 'number' },
        },
        required: ['table'],
    ...
    run: async (input, ctx) => {
      const allowedTables = ['personal_tasks', 'personal_agents', 'agent_memories', 'purchase_requests', 'agent_tasks'];
  ...
  code_exec: {
    ...
    run: async (input) => {
      ...
      if (/\b(fetch|require|import|process|globalThis|eval|Function|constructor|while|for)\b/.test(expr)) {
      ...
      try {
        ...
        return { expression: expr, value: typeof value === 'object' ? JSON.parse(JSON.stringify(value)) : value };
      } catch {
...
const DOMAIN_SCOPED = new Set(['web_fetch', 'web_search', 'browser', 'http_request', 'shopping_search', 'prepare_purchase']);
...
export async function resolveGrantedTools(
  ...
  const [{ data: perms }, { data: catalogue }] = await Promise.all([
    sb.from('tool_permissions').select('tool,enabled,requires_approval,allowed_domains,spend_cap,agent_id').in('tool', requested),
