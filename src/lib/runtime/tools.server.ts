...
import {
  BLACKSTAR_ASTRA_ORCHESTRATION_TOOL_DEF,
...
const LOCAL_TOOL_DEFS = [
  SKILL_SCRIPT_TOOL_DEF,
  SOCIAL_OPS_TOOL_DEF,
  HTML_STUDIO_TOOL_DEF,
  AGENT_WORKSPACE_TOOL_DEF,
  SEO_TOOL_DEF,
  APP_STUDIO_TOOL_DEF,
...
export const TOOL_MANIFEST = [
  ...
  { slug: "astra_orchestrate", description: BLACKSTAR_ASTRA_ORCHESTRATION_TOOL_DEF.description, sensitive: false },
];
...
export async function resolveGrantedTools(
  ...
  for (const def of requested) {
    ...
    grants.set(def.name, {
      slug: def.name,
      requiresApproval: def.name === "app_studio"
        ? Boolean(permission?.requires_approval ?? entry?.requires_approval ?? agent.requires_approval ?? false)
        : false,
      allowedDomains: [],
      spendCap: null,
    });
                    ...
                    : name === "short_video"
                      ...
                      : name === "astra_vision"
                        ...
                        : name === "astra_async_workflow"
                          ...
                          : await runBlackstarAstraOrchestrationTool(input, localCtx);
