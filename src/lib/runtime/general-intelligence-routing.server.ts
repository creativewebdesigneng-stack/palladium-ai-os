...
export async function executeAssessedGeneralIntelligence(args: {
  ...
  if (args.assessment.mode === 'escalate') {
    return {
      status: 'escalated',
      mode: 'escalate',
      reason: args.assessment.reasons.join(' '),
    }
  }
  ...
  if (args.assessment.requires_approval) {
    return {
      ...
      mode: args.assessment.mode,
      reason: 'The General Intelligence assessment requires approval before execution.',
    }
  }
  ...
  if (!selected.length) {
    return {
      status: 'failed',
      mode: args.assessment.mode,
      reason: 'No selected General Intelligence agent is authorised for this execution.',
    }
  }
  ...
  if (args.assessment.mode === 'direct') {
    try {
      const executed = await execute({
        ...
        userId: args.userId,
        agentId: selected[0]!,
        input: args.assessment.goal.objective,
      })
      ...
      return {
        status: 'failed',
        mode: 'direct',
        reason: error instanceof Error ? error.message : 'Direct General Intelligence execution failed.',
      }
    }
  }
  ...
  if (!args.plan) {
    return {
      status: 'failed',
      mode: args.assessment.mode,
      reason: 'Delegate and collective modes require a normalised authorised orchestration plan.',
    }
  }
  ...
  for (const assignment of args.plan.assignments) {
    if (!allowedForRun.has(assignment.agent_id)) {
      return {
        status: 'failed',
        mode: args.assessment.mode,
        reason: `Plan assignment ${assignment.id} targets an agent not selected by the General Intelligence assessment.`,
      }
    }
  }
