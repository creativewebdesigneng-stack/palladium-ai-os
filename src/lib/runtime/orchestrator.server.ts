...
function orchestrationPrompt(
  ...
  policy: { maxAssignments: number; forceApproval: boolean },
): ChatMessage[] {
  ...
  return [
    {
      role: "system",
      content: [
        "You are the Blackstar Orchestrator. You plan delegation only; you do not execute tools or claim tasks are complete.",
        "Break the operator goal into the smallest useful set of specialist assignments. Choose only Agent IDs in the catalogue.",
        `Use no more than ${policy.maxAssignments} assignments.`,
        policy.forceApproval
          ? "This is an assisted-autonomy mission. Every assignment must set requires_approval=true."
          : "Use requires_approval=true when the assignment itself needs an explicit human gate; existing tool-level governance remains authoritative.",
        ...
        '{"summary":"...","assignments":[{"id":"research","title":"Research market","objective":"...","agent_id":"uuid","depends_on":[],"success_criteria":["..."],"requires_approval":false}]}',
      ].join("\n"),
    },
    {
      role: "user",
...
async function createDelegationPlan(
  goal: string,
  candidates: EligibleAgentRow[],
  options: { maxAssignments: number; forceApproval: boolean },
): Promise<OrchestratorPlan> {
  const maxAssignments = Math.min(Math.max(Number(options.maxAssignments) || 1, 1), 12);
  const shortlist = shortlistAgents(goal, candidates, Math.max(maxAssignments, 1));
  ...
  const fallback = fallbackOrchestratorPlan(goal, primary, options.forceApproval);
  ...
  try {
    const result = await runChat({
      provider,
      model,
      messages: orchestrationPrompt(goal, shortlist, { maxAssignments, forceApproval: options.forceApproval }),
      tools: [],
      temperature: 0.1,
      maxTokens: 2400,
    });
...
async function persistGeneratedWorkflow(args: {
  ...
  const { data: workflow, error } = await args.sb
    ...
    .insert({
      ...
      description: args.plan.summary || "Generated by the Blackstar Orchestrator.",
      status: "active",
...
export async function planOrchestratedGoal(args: {
  ...
  const plan = await createDelegationPlan(goal, eligible.agents, {
    maxAssignments: Math.min(Math.max(Number(args.maxAssignments ?? 12) || 1, 1), 12),
    forceApproval: args.forceApproval === true,
  });
