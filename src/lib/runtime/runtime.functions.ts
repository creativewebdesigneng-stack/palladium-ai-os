...
export const runAgentTask = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      run = await prepareRun({ sb, userId: context.userId, agentId: data.agent_id, input: data.input });
...
export const cancelAgentTask = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: task } = await sb
      .from('agent_tasks')
      .update({ status: 'cancelled', completed_at: new Date().toISOString(), error: 'Cancelled by the operator.' })
      .eq('id', data.task_id)
...
export const reapStuckRuns = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabaseAdmin.rpc('reap_stale_agent_tasks', { _user: context.userId } as never);
