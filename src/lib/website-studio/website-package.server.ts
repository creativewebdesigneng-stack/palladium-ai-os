...
import { buildWebsiteAuthRuntime } from '@/lib/website-studio/website-auth-runtime.server';
...
export async function buildWebsiteRuntimePackage(sb:PackageSb,project:any):Promise<WebsiteRuntimePackage>{
  ...
  const auth=buildWebsiteAuthRuntime(project.app_config||{},project.id);
  ...
  return {
    ...
    authRuntimeEnabled:auth.enabled,
