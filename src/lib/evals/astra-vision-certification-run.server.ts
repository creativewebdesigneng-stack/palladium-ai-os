...
function parseJudge(raw: string) {
  ...
  return parsed.map((entry) => {
    ...
    if (!Number.isInteger(index) || index < 0 || index > 1 || !Number.isFinite(score) || score < 0 || score > 100) {
...
export async function runTrustedAstraVisionCertificationCase(input: RunInput) {
  ...
  const { data: run, error: runError } = await db.from('model_eval_runs').insert({
    ...
    status: 'running',
    judge_provider: input.judge.provider,
  ...
  try {
    ...
    const completedMetadata = {
      ...runMetadata,
      astra_activation: { ...runMetadata.astra_activation, server_verified: true, provenance_signature: provenanceSignature },
    }
