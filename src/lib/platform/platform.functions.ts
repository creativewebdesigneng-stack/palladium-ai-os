...
async function requireOrgRole(sb: Sb, orgId: string, userId: string, allowed: readonly string[]) {
  ...
  if (!data || !allowed.includes(data.role)) {
    await writeAudit({ userId, orgId, action: 'permission_denied', targetType: 'organisation', targetId: orgId, status: 'denied' });
...
export const getWorkspace = createServerFn({ method: 'POST' })
  ...
  .inputValidator((input: unknown) => z.object({ orgId: z.string().uuid().nullish() }).parse(input ?? {}))
  .handler(async ({ data, context }) => {
    ...
    const [{ data: profile }, { data: memberships }, { data: unread }] = await Promise.all([
      ...
      sb
        .from('organisation_members')
        .select('id,role,created_at,org_id,organisations(id,name,slug,logo_url,billing_email,created_at)')
        .eq('user_id', userId)
    ...
    const activeOrg = requested && orgs.some((o: { id: string }) => o.id === requested) ? requested : null;
...
export const createOrganisation = createServerFn({ method: 'POST' })
  ...
  .inputValidator((input: unknown) =>
    z.object({ name: z.string().trim().min(2).max(80), billingEmail: z.string().trim().email().optional() }).parse(input),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    const userId = context.userId;
    ...
    const { data: existing } = await sb.from('organisations').select('id').eq('slug', slug).maybeSingle();
    ...
    await writeAudit({ userId, orgId: org.id, action: 'organisation_created', targetType: 'organisation', targetId: org.id, metadata: { name: data.name } });
    return org;
  });
...
export const updateOrganisation = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: org, error } = await sb.from('organisations').update(patch).eq('id', data.orgId).select('id,name,slug,billing_email,logo_url').single();
    if (error) throw new Error(error.message);

    await writeAudit({ userId: context.userId, orgId: data.orgId, action: 'organisation_updated', targetType: 'organisation', targetId: data.orgId, metadata: patch });
    return org;
  });
...
export const addOrganisationMember = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!target) {
      await writeAudit({ userId, orgId: data.orgId, action: 'member_invite_failed', status: 'failed', metadata: { email: data.email } });
      throw new Error('No PalladiumAI account uses that email address yet. Ask them to sign up first.');
    }

    const { error } = await sb
      .from('organisation_members')
      .insert({ org_id: data.orgId, user_id: target.id, role: data.role, invited_by: userId });
    if (error) throw new Error(error.message.includes('duplicate') ? 'That person is already a member.' : error.message);

    await writeAudit({ userId, orgId: data.orgId, action: 'member_added', targetType: 'user', targetId: target.id, metadata: { role: data.role } });
    await notify({ userId: target.id, orgId: data.orgId, kind: 'organisation', title: 'You were added to a workspace', body: 'You now have access to a PalladiumAI organisation.', link: '/organisation' });
...
export const updateMemberRole = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) =>
    z.object({ orgId: z.string().uuid(), memberId: z.string().uuid(), role: z.enum(['owner', 'admin', 'member']) }).parse(input),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    ...
    if (data.role === 'owner' && callerRole !== 'owner') throw new Error('Only the owner can grant ownership.');

    const { error } = await sb.from('organisation_members').update({ role: data.role }).eq('id', data.memberId).eq('org_id', data.orgId);
    if (error) throw new Error(error.message);

    await writeAudit({ userId: context.userId, orgId: data.orgId, action: 'member_role_changed', targetType: 'organisation_member', targetId: data.memberId, metadata: { role: data.role } });
    return { ok: true };
  });

export const removeMember = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ orgId: z.string().uuid(), memberId: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    await requireOrgRole(sb, data.orgId, context.userId, ['owner', 'admin']);

    const { data: target } = await sb.from('organisation_members').select('role,user_id').eq('id', data.memberId).eq('org_id', data.orgId).maybeSingle();
    if (target?.role === 'owner') throw new Error('The organisation owner cannot be removed.');

    const { error } = await sb.from('organisation_members').delete().eq('id', data.memberId).eq('org_id', data.orgId);
    if (error) throw new Error(error.message);

    await writeAudit({ userId: context.userId, orgId: data.orgId, action: 'member_removed', targetType: 'organisation_member', targetId: data.memberId });
    return { ok: true };
  });
...
export const saveTeam = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload = { org_id: data.orgId, name: data.name, description: data.description ?? null, created_by: context.userId };
    const q = data.id
    ...
    await writeAudit({ userId: context.userId, orgId: data.orgId, action: data.id ? 'team_updated' : 'team_created', targetType: 'team', targetId: team.id });
    return team;
  });

export const deleteTeam = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ orgId: z.string().uuid(), id: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const sb = context.supabase as unknown as Sb;
    await requireOrgRole(sb, data.orgId, context.userId, ['owner', 'admin']);
    const { error } = await sb.from('teams').delete().eq('id', data.id).eq('org_id', data.orgId);
    if (error) throw new Error(error.message);
    await writeAudit({ userId: context.userId, orgId: data.orgId, action: 'team_deleted', targetType: 'team', targetId: data.id });
    return { ok: true };
  });
...
export const listPlans = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  const { data, error } = await supabasePublic
    .from('plans')
    .select('code,name,description,price_pence,currency,billing_interval,features,limits,sort_order')
    .eq('is_active', true)
...
export const getUsageSummary = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const base = sb.from('usage_records').select('metric,quantity,unit,occurred_at').order('occurred_at', { ascending: false }).limit(500);
    const { data: rows, error } = orgId ? await base.eq('org_id', orgId) : await base.eq('user_id', context.userId);
    if (error) throw new Error(error.message);

    const totals: Record<string, number> = {};
    for (const row of rows ?? []) totals[row.metric] = (totals[row.metric] ?? 0) + Number(row.quantity ?? 0);

    return { totals, recent: (rows ?? []).slice(0, 50), entitlements: await getEntitlements(sb as never, context.userId, orgId) };
  });
