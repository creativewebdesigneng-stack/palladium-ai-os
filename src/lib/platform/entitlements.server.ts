...
export async function getEntitlements(sb: Sb, userId: string, orgId: string | null = null): Promise<Entitlements> {
  const subQuery = sb
    .from('subscriptions')
  ...
  const [{ data: plan }, agentCount, taskCount, seatCount] = await Promise.all([
    ...
    orgId
      ? sb.from('personal_agents').select('id', { count: 'exact', head: true }).eq('org_id_fk', orgId)
      : sb.from('personal_agents').select('id', { count: 'exact', head: true }).eq('user_id', userId),
    (orgId
    ...
    orgId
      ? sb.from('organisation_members').select('id', { count: 'exact', head: true }).eq('org_id', orgId)
...
export function assertWithinLimit(ent: Entitlements, metric: keyof PlanLimits) {
  ...
  const used =
    metric === 'agents' ? ent.usage.agents : metric === 'tasks_per_month' ? ent.usage.tasksThisMonth : metric === 'seats' ? ent.usage.seats : 0;
  if (used >= limit) {
