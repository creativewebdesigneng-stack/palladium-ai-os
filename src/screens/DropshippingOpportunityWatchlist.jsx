...
import {listDropshippingOpportunities,listDropshippingOpportunitySnapshots,recordDropshippingOpportunitySnapshot,saveDropshippingOpportunity,updateDropshippingOpportunityMonitor,updateDropshippingOpportunityStatus} from '@/lib/dropshipping/dropshipping-opportunities.functions';
...
export default function DropshippingOpportunityWatchlist(){
  ...
  const monitorFn=useServerFn(updateDropshippingOpportunityMonitor);
  ...
  const monitor=useMutation({mutationFn:({id,enabled,interval_hours})=>monitorFn({data:{id,enabled,interval_hours}}),onSuccess:(row)=>{qc.invalidateQueries({queryKey:['dropshipping-opportunities']});toast({title:row.monitor_enabled?'Continuous monitoring enabled':'Continuous monitoring stopped',description:row.monitor_enabled?`Blackstar will recheck this opportunity about every ${row.monitor_interval_hours} hour${row.monitor_interval_hours===1?'':'s'} using live evidence.`:'Scheduled rechecks are disabled for this opportunity.'});},onError:error=>toast({variant:'destructive',title:'Could not update monitoring',description:friendlyMessage(error)})});
  ...
  return <section className={panel}>
    ...
    <div className="mt-5 grid gap-3 lg:grid-cols-2">{rows.map(row=>{
      ...
      return <article key={row.id} className="rounded-xl border border-white/[.08] bg-black/20 p-4"><div className="flex items-start gap-3"><div className="min-w-0"><p className="truncate text-xs font-semibold text-white">{row.title}</p><p className="mt-1 text-[10px] text-zinc-600">{row.kind} · {row.market||'market unset'} · {row.channel||'channel unset'}</p></div><span className="ml-auto rounded-full border border-white/10 px-2 py-1 text-[9px] uppercase tracking-wide text-zinc-300">{row.status}</span></div><div className="mt-3 flex items-end gap-3"><div><p className="text-[9px] uppercase tracking-wide text-zinc-600">Score</p><p className="text-2xl font-semibold text-white">{Number(row.opportunity_score??0).toFixed(1)}</p></div><p className="pb-1 text-[10px] text-zinc-500">{watchlistBand(Number(row.opportunity_score??0))}</p></div>{row.notes&&<p className="mt-2 line-clamp-3 text-[10px] leading-4 text-zinc-500">{row.notes}</p>}<div className="mt-3 rounded-lg border border-white/[.06] bg-white/[.02] p-2.5"><div className="flex items-center gap-2 text-[10px] text-zinc-500"><History className="h-3 w-3"/><span>{history.length} saved check{history.length===1?'':'s'}</span>{trend.direction!=='unknown'&&<span className={trend.direction==='rising'?'text-emerald-300':trend.direction==='falling'?'text-rose-300':'text-zinc-400'}>· {trend.direction}{trend.delta!=null?` ${trend.delta>0?'+':''}${trend.delta}`:''}</span>}</div>{latest?.research_report&&<p className="mt-2 line-clamp-3 text-[10px] leading-4 text-zinc-500">{latest.research_report}</p>}{latest&&<p className="mt-1 text-[9px] text-zinc-600">{latest.source_count||0} cited sources · {new Date(latest.checked_at).toLocaleString()}</p>}{row.monitor_enabled&&<p className="mt-1 text-[9px] text-emerald-300">Continuous monitor · every {row.monitor_interval_hours||24}h{row.next_check_at?` · next ${new Date(row.next_check_at).toLocaleString()}`:''}</p>}{row.monitor_last_error&&<p className="mt-1 line-clamp-2 text-[9px] text-amber-300">Last monitor issue: {row.monitor_last_error}</p>}</div><div className="mt-3 flex flex-wrap items-center gap-2"><select aria-label="Monitoring interval" className="rounded-lg border border-white/10 bg-[#11131a] px-2 py-1.5 text-[10px] text-zinc-300" value={row.monitor_interval_hours||24} onChange={e=>monitor.mutate({id:row.id,enabled:true,interval_hours:Number(e.target.value)})}><option value="1">Monitor hourly</option><option value="6">Every 6h</option><option value="12">Every 12h</option><option value="24">Daily</option><option value="72">Every 3 days</option><option value="168">Weekly</option></select><button disabled={monitor.isPending} onClick={()=>monitor.mutate({id:row.id,enabled:!row.monitor_enabled,interval_hours:row.monitor_interval_hours||24})} className={`rounded-lg border px-2 py-1.5 text-[10px] ${row.monitor_enabled?'border-emerald-400/20 text-emerald-200':'border-white/10 text-zinc-300'} disabled:opacity-40`}>{row.monitor_enabled?'Monitoring on':'Enable monitor'}</button><button disabled={Boolean(rechecking)} onClick={()=>recheck(row)} className="rounded-lg border border-fuchsia-400/20 px-2 py-1.5 text-[10px] text-fuchsia-200 disabled:opacity-40">{rechecking===row.id?<Loader2 className="mr-1 inline h-3 w-3 animate-spin"/>:<Sparkles className="mr-1 inline h-3 w-3"/>}AI recheck</button><button onClick={()=>setStatus.mutate({id:row.id,status:'testing'})} className="rounded-lg border border-violet-400/20 px-2 py-1.5 text-[10px] text-violet-200"><TestTube2 className="mr-1 inline h-3 w-3"/>Testing</button><button onClick={()=>setStatus.mutate({id:row.id,status:'winner'})} className="rounded-lg border border-emerald-400/20 px-2 py-1.5 text-[10px] text-emerald-200"><Trophy className="mr-1 inline h-3 w-3"/>Winner</button><button onClick={()=>promote(row)} className="rounded-lg border border-sky-400/20 px-2 py-1.5 text-[10px] text-sky-200">Promote <ArrowRight className="ml-1 inline h-3 w-3"/></button></div></article>;})}</div>
