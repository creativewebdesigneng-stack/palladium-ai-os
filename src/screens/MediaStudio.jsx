...
export default function MediaStudio() {
  ...
  return <>
    <PageHeader eyebrow="AI Workforce" title="Media Studio" description="Automatic silence and motion editing with a real Auto-Editor-compatible worker. Jobs remain persisted and auditable; PalladiumAI never fabricates rendered media." />
    ...
    <div className="grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <section className="rounded-2xl border border-white/10 bg-white/[.03] p-5">
        ...
        <div className={`mt-4 rounded-xl border p-3 text-xs ${configured ? 'border-emerald-400/20 bg-emerald-400/[.05] text-emerald-200' : 'border-amber-400/20 bg-amber-400/[.05] text-amber-200'}`}>
          <div className="flex items-center gap-2">{configured ? <CheckCircle2 className="h-4 w-4" /> : <TriangleAlert className="h-4 w-4" />}<span>{configured ? 'Auto-Editor worker configured' : 'Auto-Editor worker not configured'}</span></div>
          {!configured && <p className="mt-1 text-[11px] text-amber-200/70">Set AUTO_EDITOR_WORKER_URL on the deployment before media jobs can run.</p>}
        </div>
