...
import {
  ...
  generateGameFoundryRequiredAssets,
...
export default function GameFoundry() {
  ...
  const generateRequiredAssetsFn = useServerFn(generateGameFoundryRequiredAssets);
  ...
  const generateRequiredAssets = useMutation({
    mutationFn:(id)=>generateRequiredAssetsFn({data:{id,limit:6}}),
    onSuccess:async(res)=>{ await refresh(); const ok=res.results.filter((item)=>item.status!=='failed').length; toast({title:'Required 3D assets submitted',description:`${ok} of ${res.results.length} requirement(s) accepted by workers.`}); },
    onError:async(error)=>{ await refresh(); toast({variant:'destructive',title:'Could not generate required assets',description:friendlyMessage(error)}); },
  });
    ...
    <div className="grid gap-4 2xl:grid-cols-[1.05fr_.95fr]">
      <section className="rounded-2xl border border-violet-300/15 bg-black/30 p-5">
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-white/[.025] p-3 text-xs text-zinc-400">
          Blackstar native game compiler: <strong className="text-emerald-300">ready</strong>. External executable build worker: <strong className={caps?.gameGeneration?.externalWorkerConfigured?'text-emerald-300':'text-zinc-400'}>{caps?.gameGeneration?.externalWorkerConfigured?'connected':'optional / not configured'}</strong>. Web targets can run in Blackstar's bounded preview; Unity, Unreal, Godot and Blender targets compile portable engine source/packages without pretending engine binaries were built.
        </div>
      </section>
