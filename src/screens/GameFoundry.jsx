  ...
  return <>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-2">
      <History title="Game projects" icon={Gamepad2} empty="No Game Foundry projects yet.">
        {projects.map((project)=><div key={project.id} className="rounded-xl border border-white/10 bg-black/20 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-white">{project.name}</p><p className="mt-1 text-xs text-zinc-500">{labelize(project.project_type)} · {labelize(project.target_engine)} · {labelize(project.quality_profile)}</p></div><Status value={project.status}/></div><p className="mt-2 line-clamp-2 text-xs text-zinc-400">{project.prompt}</p>{project.design_spec?.concept&&<div className="mt-3 rounded-lg border border-cyan-300/10 bg-cyan-400/[.025] p-3"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-cyan-300/60">Compiled game design</p><p className="mt-1 text-xs text-zinc-300">{project.design_spec.concept}</p><div className="mt-2 grid gap-2 md:grid-cols-2">{project.design_spec.coreLoop?.slice(0,3).map((item)=><div key={item} className="rounded-md border border-white/[.06] bg-black/20 px-2 py-1.5 text-[11px] text-zinc-400">{item}</div>)}</div></div>}{project.source_manifest?.files?.length>0&&<div className="mt-3 rounded-lg border border-cyan-300/10 bg-cyan-400/[.025] p-3"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-cyan-300/60">Generated engine source</p><p className="mt-1 text-xs text-zinc-300">{project.source_manifest.summary}</p><div className="mt-2 flex flex-wrap gap-1.5">{project.source_manifest.files.slice(0,8).map((file)=><span key={file.path} className="rounded-md border border-white/[.06] bg-black/20 px-2 py-1 font-mono text-[10px] text-zinc-500">{file.path}</span>)}</div></div>}{project.source_error&&<p className="mt-2 text-xs text-amber-300">{project.source_error}</p>}{project.export_manifest?.assets?.length>0&&<div className="mt-3 rounded-lg border border-violet-300/10 bg-violet-400/[.025] p-3"><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-violet-300/60">Engine handoff manifest</p><p className="mt-1 text-xs text-zinc-400">{project.export_manifest.assets.length} linked asset{project.export_manifest.assets.length===1?'':'s'} · import root {project.export_manifest.importRoot}</p><p className="mt-1 text-[11px] text-zinc-500">Status: {labelize(project.handoff_status||'not_started')}{project.handoff_id?` · ${project.handoff_id}`:''}</p></div>}{project.handoff_error&&<p className="mt-2 text-xs text-amber-300">{project.handoff_error}</p>}{project.error_message&&<p className="mt-2 text-xs text-rose-300">{project.error_message}</p>}<div className="mt-3 flex flex-wrap gap-2">{['draft','failed'].includes(project.status)&&<button disabled={planProject.isPending} onClick={()=>planProject.mutate(project.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-300/20 px-2.5 py-1.5 text-xs text-cyan-200 disabled:opacity-40"><Sparkles className="h-3.5 w-3.5"/>Generate design plan</button>}{project.status==='planned'&&['not_started','failed'].includes(project.source_status||'not_started')&&<button disabled={generateSource.isPending} onClick={()=>generateSource.mutate(project.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-300/20 px-2.5 py-1.5 text-xs text-cyan-200 disabled:opacity-40"><Sparkles className="h-3.5 w-3.5"/>Generate engine source</button>}{project.status==='planned'&&<button disabled={!caps?.gameGeneration?.configured || generateProject.isPending} onClick={()=>generateProject.mutate(project.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-violet-300/20 px-2.5 py-1.5 text-xs text-violet-200 disabled:opacity-40"><Play className="h-3.5 w-3.5"/>Build game</button>}<button disabled={prepareHandoff.isPending} onClick={()=>prepareHandoff.mutate(project.id)} className="rounded-lg border border-cyan-300/20 px-2.5 py-1.5 text-xs text-cyan-200 disabled:opacity-40">Prepare engine handoff</button>{project.handoff_status==='prepared'&&<button disabled={sendHandoff.isPending || !integrations.find((item)=>item.id===project.target_engine)?.bridgeConfigured} onClick={()=>sendHandoff.mutate(project.id)} className="rounded-lg border border-violet-300/20 px-2.5 py-1.5 text-xs text-violet-200 disabled:opacity-40">Send to configured bridge</button>}{project.preview_url&&<a href={project.preview_url} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-300/20 px-2.5 py-1.5 text-xs text-emerald-300">Play preview</a>}{project.output_url&&<a href={project.output_url} target="_blank" rel="noreferrer" className="rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-zinc-300">Open project output</a>}</div></div>)}
      </History>
