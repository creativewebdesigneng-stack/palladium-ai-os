...
export default function DropshippingConnectionMatrix(){
  ...
  return <section className={`${panel} mt-5`}>
    ...
    {data&&<>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-emerald-400/15 bg-emerald-500/[.03] p-3 text-[10px] leading-5 text-zinc-500"><CheckCircle2 className="mr-1 inline h-3 w-3 text-emerald-300"/>Shopify, Etsy, eBay and WooCommerce can surface bounded built-in Blackstar actions when the user's corresponding connection exists. Amazon Seller and other providers become executable only when their actual connected integration advertises deployed actions; Blackstar does not fake SP-API signing or supplier APIs.</div><div className="rounded-xl border border-white/[.08] bg-black/20 p-3 text-[10px] leading-5 text-zinc-600"><Unplug className="mr-1 inline h-3 w-3"/>“Need connection” does not mean unsupported forever; it means Blackstar currently has no executable capability evidence for this account/provider. Credentials remain in Integrations and are never entered into the Dropshipping Hub.</div></div>
    </>}
  </section>;
}
