...
export default function AstraCertificationWorkbench() {
  ...
  const benchmark = useQuery({
    queryKey: ['astra-certification-benchmark', taskClass],
    queryFn: () => benchmarkFn({ data: { taskClass } }),
    enabled: session === 'yes',
    retry: false,
  });
  const status = useQuery({
    queryKey: ['astra-certification-status', taskClass],
    queryFn: () => statusFn({ data: { taskClass } }),
    enabled: session === 'yes',
    retry: false,
  });
    ...
    mutationFn: async (benchmarkCase) => {
      ...
      const run = await runFn({
        data: {
          ...
          astraTaskClass: taskClass,
        },
      });
      const attested = await attestFn({
        data: {
          taskClass,
          runId: run.runId,
    ...
    <section className="mb-6 rounded-2xl border border-cyan-400/20 bg-cyan-400/[.035] p-5">
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-right">
          <p className="text-[10px] uppercase tracking-wide text-zinc-500">Trusted cases</p>
          <p className="mt-1 text-xl font-semibold text-white">{completed}/{total}</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[180px_1fr_1fr]">
        <label className="text-[10px] font-medium uppercase tracking-wide text-zinc-500">
          Task class
          <select value={taskClass} onChange={(event) => setTaskClass(event.target.value)} className="input mt-1.5 w-full">
            {TASK_CLASSES.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
      ...
      {!supported && benchmark.data && (
        <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-400/[.05] p-3 text-xs text-amber-200">
          {benchmark.data.reason ?? 'This task class does not yet have a trusted certification benchmark.'}
        </div>
      )}
      ...
      {supported && benchmark.data?.cases?.length > 0 && (
        <>
          <div className="mt-4 grid max-h-72 gap-2 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-4">
            {benchmark.data.cases.map((benchmarkCase) => (
              <button
                key={benchmarkCase.caseId}
                type="button"
                disabled={benchmarkCase.completed || !canRun || runCase.isPending}
                onClick={() => runCase.mutate(benchmarkCase)}
                className="rounded-xl border border-white/10 bg-black/20 p-3 text-left transition hover:border-cyan-400/30 hover:bg-cyan-400/[.04] disabled:cursor-not-allowed disabled:opacity-45"
              >
                <div className="flex items-center gap-2">
                  {benchmarkCase.completed ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" /> : activeCaseId === benchmarkCase.caseId ? <Loader2 className="h-3.5 w-3.5 animate-spin text-cyan-300" /> : <FlaskConical className="h-3.5 w-3.5 text-cyan-300" />}
                  <p className="min-w-0 flex-1 truncate text-[11px] font-medium text-white">{benchmarkCase.name}</p>
                </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              type="button"
              disabled={!nextCase || !canRun || runCase.isPending}
              onClick={() => nextCase && runCase.mutate(nextCase)}
              className="inline-flex items-center gap-2 rounded-xl border border-cyan-400/20 bg-cyan-400/[.08] px-4 py-2.5 text-xs font-medium text-cyan-100 disabled:cursor-not-allowed disabled:opacity-40"
            >
              {runCase.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FlaskConical className="h-3.5 w-3.5" />}
              {runCase.isPending ? 'Running and attesting…' : nextCase ? 'Run next trusted case' : 'All trusted cases complete'}
            </button>
            <button
              type="button"
              disabled={!status.data?.readyToCertify || certify.isPending}
              onClick={() => certify.mutate()}
              className="inline-flex items-center gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/[.07] px-4 py-2.5 text-xs font-medium text-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
            >
              {certify.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ShieldCheck className="h-3.5 w-3.5" />}
              {certify.isPending ? 'Issuing evidence…' : status.data?.readyToCertify ? 'Issue verified evidence' : `Need ${Math.max(0, (status.data?.minimumRuns ?? 20) - (status.data?.completedRuns ?? 0))} more cases`}
            </button>
  ...
  return (
    <label className="text-[10px] font-medium uppercase tracking-wide text-zinc-500">
      ...
      <div className="mt-1.5 grid grid-cols-[130px_1fr] gap-2">
        <select value={value.provider} onChange={(event) => onChange({ ...value, provider: event.target.value })} className="input">
