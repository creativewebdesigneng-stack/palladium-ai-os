...
function Ring({ value }) {
  ...
  return (
    <svg viewBox="0 0 130 130" className="h-44 w-44 -rotate-90">
      ...
      <motion.circle cx="65" cy="65" r={r} fill="none" stroke="url(#sg)" strokeWidth="10" strokeLinecap="round"
...
export default function SecurityScore() {
  return (
    <div>
      <SectionHead icon={ShieldCheck} title="Security Score" grad="from-emerald-500 to-teal-500" action={
        <button className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-violet-600 to-indigo-600 px-3 py-1.5 text-xs font-medium text-white"><Sparkles className="h-3.5 w-3.5" />Run check</button>
      } />
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/[.025] p-6 text-center">
          <div className="relative grid place-items-center">
            ...
            <div className="absolute text-center">
              ...
              <p className="text-[10px] uppercase tracking-wide text-zinc-500">Good</p>
            </div>
          </div>
          <p className="mt-3 text-xs text-zinc-400">Overall security posture</p>
          <p className="mt-1 text-[10px] text-emerald-400">+4 points since last check</p>
        </div>
            ...
            {SCORE_BREAKDOWN.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="mb-2 flex items-center gap-2">
                  <span className={`grid h-7 w-7 place-items-center rounded-lg bg-gradient-to-br ${s.grad}`}><s.icon className="h-3.5 w-3.5 text-white" /></span>
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-white/5">
                  <motion.div className={`h-full rounded-full bg-gradient-to-r ${s.grad}`} initial={{ width: 0 }} animate={{ width: `${s.value}%` }} transition={{ duration: 0.8 }} />
                </div>
