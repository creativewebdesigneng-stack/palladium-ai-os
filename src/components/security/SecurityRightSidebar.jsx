import { History, BarChart3, Sparkles, Bell } from 'lucide-react';
import { RIGHT_SIDEBAR } from './securityData';
...
export default function SecurityRightSidebar() {
  ...
  return (
    <div className="space-y-3">
          ...
          {s.alerts.map((a) => (
            <div key={a.id} className="flex items-start gap-2 rounded-lg border border-white/5 bg-white/[.02] p-2">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5"><p className="truncate text-[11px] font-medium text-white">{a.title}</p></div>
                <p className="truncate text-[9px] text-zinc-600">{a.time}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Panel icon={BarChart3} title="Security Score" grad="from-emerald-500 to-teal-500">
        ...
        <Sparkline data={s.usage.trend} grad="from-emerald-500 to-teal-500" className="h-10" />
        <p className="mt-1 text-[10px] text-zinc-600">posture trend</p>
      </Panel>
      ...
      <Panel icon={Sparkles} title="Quick Actions" grad="from-violet-500 to-fuchsia-500">
