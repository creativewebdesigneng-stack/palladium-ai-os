  ...
  return (
    <section className="rounded-[24px] border border-white/[.08] bg-white/[.02] p-5 md:p-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div><div className="flex items-center gap-2"><FlaskConical className="h-4 w-4 text-fuchsia-300" /><h2 className="font-medium text-white">Quick paper calculator</h2></div><p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-500">Run disposable market/limit scenarios against a user-entered reference price. Persistent paper trades and journaling live in the Trader Workspace below; this quick calculator cannot contact a broker.</p></div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <div className="rounded-2xl border border-white/[.07] bg-black/20 p-4">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="flex items-center gap-2"><BookOpenCheck className="h-3.5 w-3.5 text-fuchsia-300" /><h3 className="text-xs font-semibold text-white">Session journal</h3></div><p className="mt-1 text-[10px] text-zinc-600">Disposable scenarios remain in this browser session by design. Use the persistent Trader Workspace for saved paper simulations and journal records.</p></div><label className="text-[9px] uppercase tracking-[.1em] text-zinc-600">Mark price<input type="number" min="0" step="any" value={mark} onChange={(event) => setMark(event.target.value)} className="ml-2 w-28 rounded-lg border border-white/10 bg-black/25 px-2 py-1.5 text-xs text-white outline-none" /></label></div>
