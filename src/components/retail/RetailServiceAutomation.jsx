...
import { Bot, CheckCircle2, ExternalLink, Loader2, MessageSquareText, PhoneCall, Play, Save, ShieldCheck, XCircle } from 'lucide-react';
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-violet-300/10 bg-[linear-gradient(145deg,rgba(14,11,20,.9),rgba(6,6,10,.96))] p-5 shadow-[0_28px_90px_rgba(0,0,0,.28)] backdrop-blur-xl sm:p-6">
      ...
      {tab === 'actions' && <div className="mt-5 grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
        ...
        <ActionQueue actions={data?.actions ?? []} busy={busy} onReview={review} onExecute={execute}/>
      </div>}
...
function ActionQueue({ actions, busy, onReview, onExecute }) { return <div className="rounded-2xl border border-white/[.06] bg-black/25 p-4"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold text-white">Governed action queue</h3><p className="mt-1 text-[11px] text-zinc-600">Approval and execution are distinct states.</p></div><ShieldCheck className="h-4 w-4 text-violet-300"/></div><div className="mt-4 max-h-[620px] space-y-2 overflow-auto pr-1">{actions.length?actions.map((item)=><div key={item.id} className="rounded-xl border border-white/[.06] bg-black/25 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-medium text-zinc-200">{ACTION_LABELS[item.action_type] ?? item.action_type}</p><p className="mt-1 text-[10px] text-zinc-600">{item.summary}</p></div><Status value={item.status}/></div>{item.last_error&&<p className="mt-2 rounded-lg bg-rose-400/[.05] px-2 py-1.5 text-[10px] text-rose-300">{item.last_error}</p>}<div className="mt-3 flex flex-wrap gap-2">{item.status==='pending_review'&&<><button onClick={()=>onReview(item.id,'approved')} disabled={busy===`review-${item.id}`} className="inline-flex items-center gap-1 rounded-lg border border-emerald-400/15 px-2.5 py-1.5 text-[10px] text-emerald-300"><CheckCircle2 className="h-3 w-3"/>Approve</button><button onClick={()=>onReview(item.id,'dismissed')} disabled={busy===`review-${item.id}`} className="inline-flex items-center gap-1 rounded-lg border border-rose-400/15 px-2.5 py-1.5 text-[10px] text-rose-300"><XCircle className="h-3 w-3"/>Dismiss</button></>}{item.status==='approved'&&<button onClick={()=>onExecute(item.id)} disabled={busy===`execute-${item.id}`} className="inline-flex items-center gap-1 rounded-lg bg-violet-300 px-2.5 py-1.5 text-[10px] font-semibold text-black">{busy===`execute-${item.id}`?<Loader2 className="h-3 w-3 animate-spin"/>:<Play className="h-3 w-3"/>}Execute approved action</button>}</div></div>):<p className="rounded-xl border border-dashed border-white/[.07] p-8 text-center text-xs text-zinc-600">No receptionist actions yet.</p>}</div></div>; }
function CommunicationLedger({ communications }) { return <div className="rounded-2xl border border-white/[.06] bg-black/25 p-4"><h3 className="text-sm font-semibold text-white">Communication ledger</h3><p className="mt-1 text-[11px] text-zinc-600">Provider IDs and sent states appear only after real execution.</p><div className="mt-4 max-h-[620px] space-y-2 overflow-auto pr-1">{communications.length?communications.map((item)=><div key={item.id} className="rounded-xl border border-white/[.06] bg-black/25 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs text-zinc-300">{item.channel.replaceAll('_',' ')} · {item.purpose.replaceAll('_',' ')}</p><Status value={item.status}/></div><p className="mt-1 text-[10px] text-zinc-600">{item.recipient}</p><p className="mt-2 line-clamp-2 text-[11px] leading-5 text-zinc-400">{item.body}</p>{item.last_error&&<p className="mt-2 text-[10px] text-amber-300">{item.last_error}</p>}</div>):<p className="rounded-xl border border-dashed border-white/[.07] p-8 text-center text-xs text-zinc-600">No customer communications yet.</p>}</div></div>; }
