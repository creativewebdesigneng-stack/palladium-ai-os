...
import RetailInboundVoicePanel from '@/components/retail/RetailInboundVoicePanel';
...
export default function RetailServiceAutomationSection() {
  ...
  return (
    <div>
      ...
      {selected && data ? <><RetailInboundVoicePanel key={`phone-${selected}`} workspaceId={selected} profiles={data.profiles || []} /><RetailReceptionistConsole key={`receptionist-${selected}`} workspaceId={selected} data={data} onChanged={refresh} /><RetailServiceAutomation key={`automation-${selected}`} workspaceId={selected} initialData={data} /></> : loading ? <div className="rounded-[28px] border border-white/[.06] bg-black/25 p-8 text-center text-xs text-zinc-600">Loading Retail service automation…</div> : null}
    </div>
  );
}
