...
export default function CinemaSceneRenderPanel({project,scene}){
  ...
  return <div className="mt-3 rounded-lg border border-white/8 bg-black/25 p-3">
    ...
    {(!seedreamReady||!ltxReady)&&<p className="mt-2 text-[10px] text-amber-200/65">{!seedreamReady?'Seedream worker required for keyframes. ':''}{!ltxReady?'LTX worker required for video segments.':''}</p>}
    ...
    {renders.length>0&&<div className="mt-2 grid gap-1.5 sm:grid-cols-2">{renders.slice(0,12).map(item=><div key={item.id} className="flex items-center justify-between gap-2 rounded-md border border-white/[.06] px-2 py-1.5"><div className="min-w-0"><p className="truncate text-[10px] text-zinc-300">{item.shot_id} · {item.stage}{item.stage==='video'?` #${item.segment_index+1}`:''}</p><p className="text-[9px] uppercase text-zinc-600">{item.status}</p></div><div className="flex gap-1">{item.media_job_id&&!['completed','failed','cancelled'].includes(item.status)&&<button onClick={()=>refresh.mutate(item.id)} className="text-zinc-500"><RefreshCw className="h-3 w-3"/></button>}{item.output_url&&<a href={item.output_url} target="_blank" rel="noreferrer" className="text-[9px] text-emerald-300">Open</a>}</div></div>)}</div>}
  </div>
}
