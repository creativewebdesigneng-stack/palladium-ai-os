...
export default function WebsiteAppScaffold({config,setConfig}){
  ...
  const addForm=()=>{if(!formName.trim())return;setConfig({...safe,forms:[...safe.forms,{name:formName.trim(),fields:[textField('name'),textField('email')],submitAction:'not-connected'}]});setFormName('')};
  const addCollection=()=>{if(!collectionName.trim())return;setConfig({...safe,collections:[...safe.collections,{name:collectionName.trim(),fields:[{name:'id',type:'uuid'},{name:'created_at',type:'timestamp'}],storage:'not-connected'}]});setCollectionName('')};
  ...
  return <section className="rounded-2xl border border-white/10 bg-white/[.025] p-4">
    ...
    <p className="mt-2 text-xs leading-5 text-zinc-500">Define the backend shape Website Studio should generate later. These are project requirements only until a database, auth and form-processing adapter is actually provisioned.</p>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-3">
      <Panel icon={FormInput} title="Forms">
        ...
        <div className="mt-2 space-y-2">{safe.forms.map((form,i)=><div key={form.name+i} className="flex items-center gap-2 rounded-lg border border-white/[.07] p-2"><span className="min-w-0 flex-1 truncate text-xs text-zinc-300">{form.name}</span><span className="text-[9px] text-zinc-600">{form.fields?.length||0} fields</span><button onClick={()=>setConfig({...safe,forms:safe.forms.filter((_,x)=>x!==i)})} className="text-zinc-700 hover:text-rose-300"><Trash2 className="h-3 w-3"/></button></div>)}</div>
      </Panel>
      ...
      <Panel icon={Database} title="Data collections">
        ...
        <div className="mt-2 space-y-2">{safe.collections.map((collection,i)=><div key={collection.name+i} className="flex items-center gap-2 rounded-lg border border-white/[.07] p-2"><span className="min-w-0 flex-1 truncate text-xs text-zinc-300">{collection.name}</span><span className="text-[9px] text-zinc-600">{collection.storage||'not-connected'}</span><button onClick={()=>setConfig({...safe,collections:safe.collections.filter((_,x)=>x!==i)})} className="text-zinc-700 hover:text-rose-300"><Trash2 className="h-3 w-3"/></button></div>)}</div>
      </Panel>
