...
import { PLANS, STATUSES, ORGS } from './usersData';

export default function UserFilters({ filters, setFilters, resultCount }) {
  const set = (k, v) => setFilters(f => ({ ...f, [k]: v }));
  const clear = () => setFilters({ q: '', plan: 'all', status: 'all', org: 'all', date: 'all' });
  const activeCount = ['plan','status','org','date'].filter(k => filters[k] !== 'all').length + (filters.q ? 1 : 0);
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input value={filters.q} onChange={e => set('q', e.target.value)} placeholder="Search name or email…" className="w-full rounded-xl border border-white/10 bg-black/30 py-2 pl-9 pr-3 text-sm text-zinc-200 placeholder:text-zinc-600 focus:border-violet-400/40 focus:outline-none" />
        </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <Select label="Organisation" value={filters.org} onChange={v => set('org', v)} options={['all', ...ORGS]} />
      ...
      <p className="text-[11px] text-zinc-500">{resultCount} users</p>
    </div>
  );
}
