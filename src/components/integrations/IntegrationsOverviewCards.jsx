...
import { OVERVIEW } from './integrationsData';
import { Sparkline } from './shared';

export default function IntegrationsOverviewCards() {
  return (
          ...
          <motion.div key={m.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.25) }} whileHover={{ y: -2 }}
            ...
            <div className="mb-2.5 flex items-center justify-between">
              ...
              <span className={`flex items-center gap-0.5 text-[10px] font-medium ${up ? 'text-emerald-400' : 'text-red-400'}`}>
                {up ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}{m.delta}
              </span>
            </div>
