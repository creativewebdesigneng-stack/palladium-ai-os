  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.03] p-5">
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <p className="text-[11px] text-zinc-500">Google Shopping + verified retailer product results</p>
        <span className={`ml-auto inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-medium ${googleConfigured ? 'border-emerald-400/25 bg-emerald-500/10 text-emerald-300' : 'border-amber-400/25 bg-amber-500/10 text-amber-300'}`}>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-white/10 bg-black/20 p-4">
          <p className="text-xs font-medium text-zinc-300">
            {latestTask
              ? `No live product cards were returned for “${latestTask.requirement}”.`
            ...
            return (
              <section key={taskId}>
                <div className="mb-3 flex items-center gap-2 text-[10px] uppercase tracking-wider text-zinc-600">
                  <PackageSearch className="h-3 w-3" />{rows.length} live option{rows.length === 1 ? '' : 's'}
                  {purchase && (
