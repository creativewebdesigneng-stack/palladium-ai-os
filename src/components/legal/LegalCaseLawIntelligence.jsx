  ...
  return <section className="rounded-[24px] border border-white/[.08] bg-white/[.02] p-5">
    ...
    {result&&<div className="mt-5 grid gap-4 xl:grid-cols-[1fr_330px]">
      ...
      <aside className="space-y-3">
        <div className="rounded-xl border border-white/[.06] bg-black/20 p-3"><div className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5 text-emerald-300"/><p className="text-xs text-zinc-300">Official evidence {result.official_source_count}/{result.source_count}</p></div><p className="mt-1 text-[10px] leading-4 text-zinc-600">Official-host status identifies source provenance only. It does not establish precedential weight or current treatment.</p></div>
        <div><p className="text-[10px] uppercase tracking-[.14em] text-zinc-600">Evidence sources</p><div className="mt-2 space-y-2">{result.sources.map(s=><a key={s.url} href={s.url} target="_blank" rel="noreferrer" className="block rounded-xl border border-white/[.06] bg-black/20 p-3 hover:border-cyan-300/20"><div className="flex gap-2"><p className="flex-1 text-xs text-zinc-300">{s.title}</p><ExternalLink className="h-3 w-3 text-zinc-700"/></div><div className="mt-2 flex items-center justify-between gap-2"><span className={s.official?'rounded-full border border-emerald-300/15 bg-emerald-300/[.05] px-2 py-0.5 text-[9px] text-emerald-200':'rounded-full border border-amber-300/15 bg-amber-300/[.04] px-2 py-0.5 text-[9px] text-amber-200'}>{s.official?'Official host':'Discovery only'}</span><p className="min-w-0 flex-1 truncate text-right text-[9px] text-zinc-700">{s.url}</p></div></a>)}</div></div>
