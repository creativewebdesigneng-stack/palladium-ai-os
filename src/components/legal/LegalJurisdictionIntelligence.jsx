...
function JurisdictionCard({ value, onChange, item }) {
  ...
  return <div className="rounded-2xl border border-white/[.07] bg-black/20 p-4">
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2"><span className="rounded-full border border-violet-300/15 bg-violet-300/[.05] px-2 py-1 text-[10px] uppercase tracking-[.12em] text-violet-200/80">{item?.region}</span><span className="rounded-full border border-cyan-300/15 bg-cyan-300/[.05] px-2 py-1 text-[10px] uppercase tracking-[.12em] text-cyan-200/80">{tier?.label}</span><span className="text-[10px] text-zinc-600">Reviewed {item?.reviewedOn}</span></div>
