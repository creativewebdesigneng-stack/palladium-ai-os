...
import { TEAM_ANALYTICS } from './analyticsData';
...
export default function TeamAnalytics() {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.03] p-4">
      ...
      <p className="text-[11px] text-zinc-500">Requests & cost by team</p>
      <div className="mt-2 h-44">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={TEAM_ANALYTICS} layout="vertical" margin={{ left: 32, right: 16 }}>
            ...
            <Bar dataKey="requests" radius={[0, 4, 4, 0]} barSize={16}>
              {TEAM_ANALYTICS.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div className="mt-2 space-y-1">
        {TEAM_ANALYTICS.map((t, i) => (
