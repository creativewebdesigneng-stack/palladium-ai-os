...
export default function AgentAnalytics({ data = [] }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.03] p-4">
      <h3 className="text-sm font-semibold text-white">Agent Analytics</h3>
      <p className="text-[11px] text-zinc-500">Tasks & success rate per agent</p>
      {data.length === 0 ? (
        <div className="mt-3 rounded-xl border border-dashed border-white/10 p-6 text-center text-xs text-zinc-500">No agent task data yet.</div>
      ) : (
        <>
