    ...
    handlers: {
      GET: withApiAuth({ scope: "agents:read", path: PATH }, async (ctx, request) => {
        ...
        const query = ctx.admin
          ...
          .select(
            "id,name,category,purpose,status,autonomy,allowed_tools,requires_approval,created_at",
          )
      ...
      POST: withApiAuth({ scope: "agents:write", path: PATH }, async (ctx, request) => {
        ...
        const { data, error } = await ctx.admin
          ...
          .insert({
            ...
            instructions:
              typeof body["instructions"] === "string" ? body["instructions"].slice(0, 8000) : null,
            allowed_tools: Array.isArray(body["allowed_tools"])
              ? body["allowed_tools"].slice(0, 40)
              : [],
