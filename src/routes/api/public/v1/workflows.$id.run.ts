...
export const Route = createFileRoute('/api/public/v1/workflows/$id/run')({
  server: {
    handlers: {
      ...
      POST: withApiAuth({ scope: 'workflows:run', execution: true, path: PATH }, async (ctx, request, params) => {
        ...
        const body = (await request.json().catch(() => ({}))) as { input?: string; objective?: string };
        const input = String(body.input ?? body.objective ?? '').trim();
        if (!input) throw new ApiError(400, 'invalid_request', 'An "input" objective is required.');
        ...
        try {
          ...
          if (error instanceof WorkforceError) throw new ApiError(400, 'run_failed', error.message);
          throw error;
        }
      }),
    },
