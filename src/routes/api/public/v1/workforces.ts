    ...
    handlers: {
      GET: withApiAuth({ scope: 'workforces:read', path: PATH }, async (ctx, request) => {
        ...
        const members = ids.length
          ? (await ctx.admin.from('workforce_agents').select('workforce_id,agent_id,role').in('workforce_id', ids)).data ?? []
          : [];

        return {
          workforces: (data ?? []).map((w: any) => ({
            ...w,
            members: members.filter((m: any) => m.workforce_id === w.id).map((m: any) => ({ agent_id: m.agent_id, role: m.role })),
          })),
          limit,
          offset,
        };
