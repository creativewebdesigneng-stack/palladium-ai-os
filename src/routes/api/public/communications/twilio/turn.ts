...
export const Route = createFileRoute('/api/public/communications/twilio/turn')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          const [{ processAiCallTurn }, { communicationsPublicUrl, verifyTwilioCommunicationsWebhook, twimlSayAndHangup }] = await Promise.all([
          ...
          const canonicalUrl = communicationsPublicUrl('/api/public/communications/twilio/turn', { session });
          ...
          return processAiCallTurn(params, session);
        } catch (error) {
          const status = typeof error === 'object' && error !== null && 'status' in error && typeof error.status === 'number' ? error.status : 500;
          console.error('[communications] Twilio AI voice turn failed', error);
