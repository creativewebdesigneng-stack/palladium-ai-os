...
import { ProviderError, runChat as runProviderChat, type Provider } from "@/lib/runtime/model-gateway.base";

export const Route = createFileRoute("/api/internal/provider-readiness-9b7e4d21")({
  server: {
    handlers: {
      GET: async () => {
        const results: Array<{ id: Provider; configured: boolean; ready: boolean; status: number | null; code: string }> = [];
