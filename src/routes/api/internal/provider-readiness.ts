...
/**
 * TEMPORARY diagnostic route used only on the protected preview branch.
 * It returns provider configuration/readiness metadata and never returns keys,
 * request headers, environment values, prompts, or provider response bodies.
 */
export const Route = createFileRoute("/api/internal/provider-readiness")({
  server: {
    handlers: {
      GET: async () => {
