...
export const Route = createFileRoute("/api/internal/openai-agent-runtime-proof-2d7a51")({
  server: {
    handlers: {
      GET: async () => {
        ...
        try {
          const { error: updateError } = await db
            ...
            .eq("id", AGENT_ID)
            .eq("user_id", USER_ID);
          if (updateError) return json({ ok: false, stage: "prepare_agent" });
          ...
          return json({ ok: false, stage: "runtime", error_type: error instanceof Error ? error.name : "unknown" });
        } finally {
          await db
            .from("personal_agents")
