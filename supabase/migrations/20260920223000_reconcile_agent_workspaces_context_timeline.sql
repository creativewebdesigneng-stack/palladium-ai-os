-- Restore Blackstar's original Agent Workspace + Context Timeline production schema.
-- Existing schema and business logic are reused; no parallel workspace subsystem.
...
-- The base schema predates strict explicit role grants. Preserve its owner-only
-- model, prevent anonymous Data API access and disallow cross-owner card links.
revoke all on public.agent_workspaces,public.context_timeline_cards from public,anon,authenticated;
grant all on public.agent_workspaces,public.context_timeline_cards to service_role;
grant select,insert,update,delete on public.agent_workspaces,public.context_timeline_cards to authenticated;

drop policy if exists "agent_workspaces_owner_select" on public.agent_workspaces;
create policy "agent_workspaces_owner_select" on public.agent_workspaces for select to authenticated
  using (user_id=(select auth.uid()));
drop policy if exists "agent_workspaces_owner_insert" on public.agent_workspaces;
create policy "agent_workspaces_owner_insert" on public.agent_workspaces for insert to authenticated
  with check (
...
drop policy if exists "agent_workspaces_owner_update" on public.agent_workspaces;
create policy "agent_workspaces_owner_update" on public.agent_workspaces for update to authenticated
  using (user_id=(select auth.uid()))
  with check (
    user_id=(select auth.uid())
    and (agent_id is null or exists (
...
drop policy if exists "agent_workspaces_owner_delete" on public.agent_workspaces;
create policy "agent_workspaces_owner_delete" on public.agent_workspaces for delete to authenticated
  using (user_id=(select auth.uid()));

drop policy if exists "context_timeline_owner_select" on public.context_timeline_cards;
create policy "context_timeline_owner_select" on public.context_timeline_cards for select to authenticated
  using (user_id=(select auth.uid()));
drop policy if exists "context_timeline_owner_insert" on public.context_timeline_cards;
create policy "context_timeline_owner_insert" on public.context_timeline_cards for insert to authenticated
  with check (
    user_id=(select auth.uid())
...
drop policy if exists "context_timeline_owner_update" on public.context_timeline_cards;
create policy "context_timeline_owner_update" on public.context_timeline_cards for update to authenticated
  using (user_id=(select auth.uid()))
  with check (
    user_id=(select auth.uid())
    and (workspace_id is null or exists (
...
drop policy if exists "context_timeline_owner_delete" on public.context_timeline_cards;
create policy "context_timeline_owner_delete" on public.context_timeline_cards for delete to authenticated
  using (user_id=(select auth.uid()));
