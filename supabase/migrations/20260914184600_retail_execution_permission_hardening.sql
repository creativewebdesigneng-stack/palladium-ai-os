...
-- The public SECURITY INVOKER wrappers require authenticated EXECUTE on private helpers,
...
-- Gift cards must be issued through an atomic operation so the initial balance always has an immutable event.
revoke insert on table public.retail_gift_cards from authenticated;
...
begin
  ...
  insert into public.retail_gift_card_events (
    ...
    v_user_id, p_workspace_id, v_card.id, 'issue', p_value, p_value, 'initial_issue', p_note
  );
