...
create or replace function public.project_repository_add_collaborator_by_email(
  p_project_id uuid,
  p_email text,
...
begin
  if not private.can_manage_project(p_project_id) then
    raise exception 'Only project owners or workspace managers can manage collaborators.';
  end if;
  if p_role not in ('viewer','contributor','maintainer') then
  ...
  if not found then
    raise exception 'No Blackstar account matches that email.';
  end if;
  if v_profile.id = (select auth.uid()) then
    raise exception 'The project owner is already a member.';
  end if;

  insert into public.project_collaborators(project_id, user_id, role, added_by)
  values (p_project_id, v_profile.id, p_role, (select auth.uid()))
  on conflict (project_id, user_id)
  do update set role = excluded.role;

  return query
  select v_profile.id, v_profile.full_name, v_profile.email, p_role;
end;
$$;

revoke all on function public.project_repository_add_collaborator_by_email(uuid,text,text) from public;
grant execute on function public.project_repository_add_collaborator_by_email(uuid,text,text) to authenticated;

create or replace function public.project_repository_list_collaborators(p_project_id uuid)
...
set search_path = pg_catalog, public
as $$
  select pc.user_id, p.full_name, p.email, pc.role, pc.created_at
...
revoke all on function public.project_repository_list_collaborators(uuid) from public;
grant execute on function public.project_repository_list_collaborators(uuid) to authenticated;
