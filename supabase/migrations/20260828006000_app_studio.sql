...
values
  ('app_studio', 'App Studio', 'development',
   'Build and test persisted low-code application drafts with pages, components, secure datasource references and queries. Publishing remains operator-controlled.',
   'builtin', false, 'medium', 'builder', true,
   '{"actions":["list_apps","get_app","create_app","add_page","add_widget","add_datasource","add_query","run_query"],"publishes":false,"credentials_server_side":true}'::jsonb)
