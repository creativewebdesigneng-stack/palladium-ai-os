...
-- The execution result is backend evidence, never a client-editable claim.
revoke all on public.agent_skill_script_executions from public,anon,authenticated;
grant select on public.agent_skill_script_executions to authenticated;
grant all on public.agent_skill_script_executions to service_role;
...
create index if not exists agent_skill_script_executions_approval_idx
  on public.agent_skill_script_executions(approval_request_id);
