...
class FakePage extends EventEmitter {
  url() { return "https://blackstar.example/ai-hub"; }
  async title() { return "AI Hub"; }
  async waitForTimeout() {}
  async screenshot() { return Buffer.from("png"); }
}

test("collects bounded read-only QA evidence and removes observers", async () => {
  ...
  const result = await inspectPageReadOnly(page, async () => {
    ...
    page.emit("requestfailed", { method: () => "GET", url: () => "https://blackstar.example/api/x", failure: () => ({ errorText: "net::ERR_FAILED" }) });
    page.emit("response", { status: () => 500, url: () => "https://blackstar.example/api/y", request: () => ({ method: () => "GET" }) });
    return { status: 200 };
  });
