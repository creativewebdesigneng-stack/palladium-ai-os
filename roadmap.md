...
- [x] 3. Opportunity execution: `recommendBlackstarOpportunityActions` ranks owner-scoped `autonomous_goals` / runs through `planBlackstarOpportunityExecution` and existing agents/workflows. Approval-gated cards use `createAiHubApprovalGate` only; listing does not execute. "Recommended actions" panel is on `src/screens/AutonomousOS.jsx`.
