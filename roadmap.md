...
- [x] 2. Computer-use gate: `runBoundedBrowserTask` preflights `buildBrowserTaskComputerUsePlan` before dispatch; trusted login/download-only tasks remain executable. Recorded sessions are scored in `getBrowserControl` and shown on `ComputerControl`. Stateful sessions still run only through that bounded runner.
...
- [x] 5. Dynamic teams: when orchestrator assignments are empty, Autonomous OS now forms a fallback team from the owner's active `personal_agents` (tools/skills + task reliability) and persists `autonomous_goal_fleet_assignments`. Hub mission planner remains the interactive planning surface.
